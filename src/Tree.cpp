...
//    if(rand() % 2 == 0){
        grammar->derivate(n);
//    }else{
//        int deep = grammar->gg.at(0)->high + (rand() % (conf->MAXDEEP - grammar->gg.at(0)->high)) + 1;
...
Tree::Tree(Grammar* gg) {
    No* n = new No(-1, 0, "<expr>", 0);
//    if(rand() % 2 == 0){
        gg->derivate(n);
//    }else{
//        int deep = grammar->gg.at(0)->high + (rand() % (conf->MAXDEEP - grammar->gg.at(0)->high)) + 1;
//        gg->createInitialPopulation(n, deep);
//    }
    root = n;
//    cout << "Criando arvore com raiz " << root->mask << endl;
    update();
...
Tree::Tree(Grammar* gg, No* n_) {
    No* n = new No(n_->type, n_->value, n_->mask, n_->deep);
...
bool Tree::compareSemantic(No* a, No* b)
{
...
vector<No*> Tree::targetSubTrees(No* n) {
    targetedNonTerminals.clear();
    targetedNonTerminalCount(n, root);

    return targetedNonTerminals;
};
...
void Tree::targetedNonTerminalCount(No* n, No* m) {
    if(n->type == m->type &&
        ...
        m->semantic_distance = m->semanticDistance(n, conf->distance_method);
        m->semantic_difference = m->semanticDifference(n);
    }
    for(No* p : m->sons)
