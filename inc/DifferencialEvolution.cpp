...
//    cout << s->fitness << endl;
    ...
    int total = 0;
    for(int i = 0; i < conf->numTree; i++){
        initialIndividual[i] = new double[ind->trees[i]->constantsSize];
        ind->trees[i]->Constants(initialIndividual[i], 0, ind->trees[i]->root);
...
////    conf->numTree = 3;
    ...
    for (int i = 0; i < p_size; i++) {
       ...
       if(i == 0){
           delete [] population[i];
           population[i] = initialIndividual;
       }
       else{
...
//    cin.get();
    ...
    while (generations * p_size < 100000) {
        generations++;
...
//        cout << "gen " << generations << " BEST " << fitness[(int)best] << endl;
//        ind->print();
//        print(population[(int)best], dim);
//        cin.get();
...
//    s->fitness = fitness[(int)best];

//    for(int i = 0; i < conf->numTree; i++) {
//        cout << i << ": ";
//        for(int j = 0; j < dim[i]; j++) {
//            cout << population[(int)best][i][j] << " ";
//        }
//        cout << endl;
//    }
...
void DifferencialEvolution::initialSolutions(double **population, int *dim, double limInf, double limSup, double **initialIndividual) {
    ...
    for(int k = 0; k < conf->numTree; k++) {
        for (int i = 0; i < dim[k]; i++) {
            population[k][i] = initialIndividual[k][i] + rands;
...
//        cout << "fit " << fitness[i] << endl;
//        ind->print();
//        for(int t = 0; t < conf->numTree; t++){
//            cout << "[";
//            for(int c = 0; c < dim[t]; c++){
//                cout << ind->constants[t][c] << ", ";
//            }
//            cout << "]";
//        }
//        cout << endl;
